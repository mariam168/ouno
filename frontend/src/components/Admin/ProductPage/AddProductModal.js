...
import { X, Plus, Trash2, Image as ImageIcon, UploadCloud, AlertCircle, ChevronsRight, Loader2, CheckCircle } from 'lucide-react';
...
const AddProductModal = ({ onClose, onProductAdded, serverUrl }) => {
    ...
    const handleMainImageSelect = (e) => { const file = e.target.files[0]; if (file) { setMainImageFile(file); setMainImageUrl(URL.createObjectURL(file)); } };
    const handleRemoveMainImage = () => { setMainImageFile(null); setMainImageUrl(''); };
    ...
    const handleSubmit = async (e) => {
        ...
        const finalVariations = variations.map(v => ({
            ...v, options: v.options.map(o => ({
                ...o, skus: o.skus.map(s => ({ ...s, sku: s.sku || null }))
            }))
        }));
        formData.append('variations', JSON.stringify(finalVariations));
        Object.keys(optionImageFiles).forEach(key => {
            formData.append(key, optionImageFiles[key]);
        });
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-50 p-4">
            <div className="bg-white dark:bg-zinc-900 p-0 rounded-2xl shadow-lg relative w-full max-w-6xl max-h-[95vh] flex flex-col border border-gray-200 dark:border-zinc-800">
                <header className="flex justify-between items-center p-4 sm:p-5 border-b border-gray-200 dark:border-zinc-800 flex-shrink-0">
                    <h2 className="text-xl font-semibold text-gray-900 dark:text-white flex items-center gap-3"><Plus className="text-indigo-500" />{t('productAdmin.addNewProduct')}</h2>
                ...
                <form id="add-product-form" onSubmit={handleSubmit} className="flex-grow overflow-y-auto p-4 sm:p-6 space-y-6">
                    ...
                    <Fieldset legend={t('productAdmin.basicInfo')}>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                            ...
                            <Input label={t('product.nameAr')} name="name_ar" value={product.name_ar} onChange={handleProductChange} dir="rtl" required/>
                            ...
                            <div className="md:col-span-2">
                                <label className="block text-sm font-medium text-gray-700 dark:text-zinc-300 mb-1.5">{t('product.mainImage')}</label>
                                <div className="flex items-center gap-4"><ImagePreview src={mainImageUrl} onRemove={handleRemoveMainImage} /><ImageUploader id="main-image-upload" onChange={handleMainImageSelect} t={t} /></div>
                            </div>
                ...
                <footer className="flex-shrink-0 flex justify-end gap-3 p-4 mt-auto border-t border-gray-200 dark:border-zinc-800">
                    ...
                    <button type="submit" form="add-product-form" disabled={isSubmitting} className="flex items-center justify-center gap-2 rounded-lg bg-gray-900 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-indigo-600 dark:bg-white dark:text-black dark:hover:bg-indigo-500 disabled:opacity-60">{isSubmitting ? <Loader2 className="animate-spin" size={20} /> : <CheckCircle size={20} />}<span>{isSubmitting ? t('actions.saving') : t('actions.saveProduct')}</span></button>
                </footer>
            </div>
        </div>
    );
